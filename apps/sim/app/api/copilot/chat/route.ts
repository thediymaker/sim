...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const selectedModel = model || 'qwen3-30b-a3b-thinking-2507'
    ...
    // -------------------------------------------------------------------------
    // If a LiteLLM Base URL is configured, bypass the Sim Agent and call
    // LiteLLM directly via the OpenAI SDK.
    ...
    if (useLiteLLM) {
      ...
      try {
        ...
        const openai = new OpenAI({
          ...
          apiKey: env.LITELLM_API_KEY || 'sk-placeholder',
        })

        // Construct messages with history
        const openaiMessages = [...conversationHistory, { role: "user", content: message }].map((m: any) => ({
          role: m.role,
          content: m.content,
        }))

        // Inject system prompt and agent contexts
...
If no context is provided, rely on your general knowledge but mention you lack specific context about their workflow.`
        ...
        const litellmStream = new ReadableStream({
          async start(controller) {
            try {
              ...
              controller.enqueue(
                encoder.encode(
                  `data: ${JSON.stringify({ type: 'done', data: { responseId } })}\n\n`
                )
              )
            } catch (e) {
              ...
              controller.enqueue(
                encoder.encode(
                  `data: ${JSON.stringify({ type: 'error', data: { displayMessage: errMsg } })}\n\n`
                )
              )
            } finally {
              controller.close()
        ...
        return new NextResponse(litellmStream, {
          headers: {
    ...
    // Fallback to Sim Agent if not using LiteLLM
